...
output_unit = eye(num_labels)(y,:);   % Equiv to (y_k) ^ (i) in Cost Fxn equation

cost = output_unit .* log(a3) + (1 - output_unit) .* log(1 - a3);
J = -sum(sum(cost, 2)) / m;

reg = sum(sum(Theta1(:, 2:end) .^ 2)) + sum(sum(Theta2(:, 2:end) .^ 2));

J = J + ((lambda/(2 * m)) * reg);     % J(THETA) Computed 

% ====================== PART 2: BACKPROP ALGORITHM ======================

delta3 = a3 - output_unit;
